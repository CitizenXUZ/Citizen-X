...
def handle_start(chat_id: int, first_name: str):
    ...
    text = (
        f"👋 Привет, <b>{name}</b>!\n"
        "\n"
        "🎓 <b>Citizen X</b> — онлайн-платформа для подготовки к <b>IELTS</b> и сертификации по <b>CEFR</b> (A1–B2).\n"
        "\n"
        ...
        f"🔗 Сайт: <a href=\"{SITE_WEB_URL}\">citizen-x.up.railway.app</a>\n"
        "📢 Канал: <a href=\"https://t.me/citizen_x_channel\">@citizen_x_channel</a>\n"
        "\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
    ...
    keyboard = build_inline_keyboard([
        ...
        [
            ...
            {"text": "📢 Наш канал", "url": "https://t.me/citizen_x_channel"},
        ],
    ])
...
def handle_text(chat_id: int, role: str, first_name: str, text: str):
    ...
    if state and state.get("state") == "awaiting_parent_self_phone":
        ...
        if status == 200 and isinstance(data, dict) and data.get("status") == "ok":
            send_message(
                chat_id,
                "✅ <b>Готово!</b>\n\nВаш Telegram подключён. Теперь вы будете получать уведомления об успехах вашего ребёнка в <b>Citizen X</b>.",
                reply_markup=build_remove_keyboard(),
            )
        elif status == 404:
            send_message(

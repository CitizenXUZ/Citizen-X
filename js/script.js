...
if (heroHeader) {
  ...
  const syncMobileSmoothVars = (y) => {
    ...
    // At ≤720px: bigger logo on load → clearly smaller compact logo.
    // When logged-in the CSS caps logo via max-width (.mobile-logged-in),
    // so we can use a generous initial size here for non-logged-in visitors.
    const isMobileSmall = vw <= 720;
    const maxLogo = isMobileSmall ? Math.min(vw * 0.72, 270) : Math.min(vw * 0.78, 330);
    const minLogo = isMobileSmall ? Math.min(vw * 0.38, 148) : Math.min(vw * 0.46, 176);
    const logoW = Math.round(maxLogo - (maxLogo - minLogo) * t);
    heroHeader.style.setProperty("--m-logo-w", `${logoW}px`);

    // Generous padding on load → clearly expanded; keep enough padding in
    // compact mode so the Menu button sits visually centred, not pinned to top
    const maxPadY = 22;
    const minPadY = 18;
    const padY = Math.round((maxPadY - (maxPadY - minPadY) * t) * 10) / 10;
    heroHeader.style.setProperty("--m-header-pad-y", `${padY}px`);

    // Button padding & font-size — white pill, kept compact to fit in 1fr cell
    const maxBtnPadY = 8;
    const minBtnPadY = 6;
  ...
  window.addEventListener("resize", () => {
    ...
    syncLogoSizeFromHeader();
    // Re-evaluate mobile-logged-in class and menu button visibility on resize/rotation
    updateLoginButton();
  });
...
const ACCESS_AUTH_STORAGE_KEY = "cx_auth_state";
const COURSE_CODES = ["a1", "a2", "b1", "b2"];

const LESSON_OVERRIDES_CACHE = {};
const MERGED_LESSONS_CACHE = {};
const ADMIN_EDIT_CODE_STORAGE_PREFIX = "cx_admin_edit_code:";
...
const getLessonCompletionStorageKey = (course, lessonNumber, username) =>
  `cx_lesson_completed:${String(course || "").toLowerCase()}:${Number(lessonNumber) || 1}:${username || "guest"}`;
...
if (tasksPage) {
  ...
  const ensureHomeworkSentModal = () => {
    ...
    if (modal) {
	  ...
	  const getHomeworkSubmittedStorageKey = (username) => {
	    ...
	    return `cx_homework_submitted:${userKey}:${course}:${lessonNumber}`;
	  };
  ...
  if (!hasQuiz) {
    ...
    window.dispatchEvent(new CustomEvent("cx:tasks-ready"));
    return;
  }
  ...
  quizQuestions = loadedQuestions;
    ...
    const getQuizStateStorageKey = () => {
      ...
      return `cx_quiz_state:${course}:${lessonNumber}:${username}`;
    };
...
const AUTH_STORAGE_KEY = "cx_auth_state";
const LEGACY_AUTH_STORAGE_KEY = "cx_auth_user";
...
const getAchievementNotifyStorageKey = (username) =>
  `cx_achievement_notify:${String(username || "").trim().toLowerCase()}`;
  ...
  if (!document.getElementById("privacy-modal")) {
    ...
    privacy.innerHTML = `
      <div class="legal-modal-card" role="dialog" aria-modal="true" aria-labelledby="privacy-title">
        ...
        <pre class="legal-text">Privacy Policy - Citizen X
Last Updated: [дата]
Welcome to Citizen X ("we," "our," or "us"), a platform operated by Sunnatullo Raxmatullaev's team. Your privacy is important to us. This Privacy Policy explains how we collect, use, store, and protect your information when you use our website and services (the "Platform"). By using our Platform, you agree to the terms outlined in this Privacy Policy.
...
const initAdminLessonsEditing = () => {
  ...
  const startGate = async () => {
    ...
    if (stored) {
      ...
      if (verify.ok && verify.data && verify.data.ok) {
        if (typeof window.__cxDailyContentCallback === "function") {
          const cb = window.__cxDailyContentCallback;
          window.__cxDailyContentCallback = null;
          await cb();
        } else {
  ...
  window.__cxOpenLessonsEditGate = triggerGate;
  ...
  try {
    ...
    if (autoLesson > 0 && window.__cxAdminAutoLessonOpened !== `${course}:${autoLesson}`) {
      window.__cxAdminAutoLessonOpened = `${course}:${autoLesson}`;
      try {
...
const renderResultsCertificates = (items) => {
  ...
  if (typeof window.__cxBuildResultsMasonry === "function") {
    window.__cxBuildResultsMasonry();
  }
};
...
const loadAndRenderCertificatesOverrides = async () => {
  const resultsEl = document.querySelector(".results-standalone");
  if (!resultsEl) {
    window.dispatchEvent(new CustomEvent("cx:certs-ready"));
    return [];
  }
  await ensureApiBaseUrl();
  try {
...
const initAdminSubscriptionsEditing = () => {
  ...
  editBtn.addEventListener("click", triggerGate);
  editBtn.onclick = triggerGate;
  window.__cxOpenSubscriptionsEditGate = triggerGate;
...
(() => {
  const tryOpenLessons = () => {
    if (typeof window.__cxOpenLessonsEditGate === "function") {
      window.__cxOpenLessonsEditGate();
      return true;
    }
  ...
  const tryOpenSubscriptions = () => {
    if (typeof window.__cxOpenSubscriptionsEditGate === "function") {
      window.__cxOpenSubscriptionsEditGate();
      return true;
    }
    try {
...
const applyLessonAccessLocks = async () => {
  ...
  // ── Level upgrade banner + modal (shown when user has no access to this level) ──
  const injectLevelUpgradeBanner = () => {
    if (lessonsPageEl.querySelector(".level-upgrade-banner")) return;
    const courseLabel = String(currentCourse || "").toUpperCase();
    const banner = document.createElement("div");
    banner.className = "level-upgrade-banner";
    banner.innerHTML = `
      <div class="level-upgrade-banner-inner">
        <div class="level-upgrade-icon">🔒</div>
        <div class="level-upgrade-text">
          <h2 class="level-upgrade-title">Upgrade to Pro</h2>
          <p class="level-upgrade-sub">Get full access to all lessons of the <strong>${courseLabel} level</strong> — unlock videos, quizzes and mentor feedback.</p>
        </div>
        <button type="button" class="btn level-upgrade-btn" id="level-upgrade-open-btn">Upgrade</button>
      </div>
    `;
    const grid = lessonsPageEl.querySelector(".lessons-grid");
    if (grid) {
      grid.insertAdjacentElement("beforebegin", banner);
    } else {
      lessonsPageEl.prepend(banner);
    }
    document.getElementById("level-upgrade-open-btn").addEventListener("click", openLevelUpgradeModal);
  };

  const ensureLevelUpgradeModal = () => {
    if (document.getElementById("level-upgrade-modal")) return;
    const courseLabel = String(currentCourse || "").toUpperCase();
    const wrap = document.createElement("div");
    wrap.innerHTML = `
      <div id="level-upgrade-modal" class="upgrade-modal" aria-hidden="true">
        <div class="upgrade-modal-card" role="dialog" aria-modal="true" aria-labelledby="level-upgrade-modal-title">
          <button type="button" class="upgrade-modal-close" id="level-upgrade-modal-close" aria-label="Close">&times;</button>
          <h3 id="level-upgrade-modal-title" class="upgrade-modal-title">${courseLabel} LEVEL — UPGRADE TO PRO</h3>
          <div class="upgrade-plan-grid" id="level-upgrade-plan-grid">
            <p class="level-upgrade-loading">Loading plans…</p>
          </div>
        </div>
      </div>
    `;
    document.body.appendChild(wrap);
    document.getElementById("level-upgrade-modal-close").addEventListener("click", closeLevelUpgradeModal);
    document.getElementById("level-upgrade-modal").addEventListener("click", (e) => {
      if (e.target === document.getElementById("level-upgrade-modal")) closeLevelUpgradeModal();
    });
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape") closeLevelUpgradeModal();
    });
  };

  const closeLevelUpgradeModal = () => {
    const m = document.getElementById("level-upgrade-modal");
    if (m) { m.classList.remove("is-open"); m.setAttribute("aria-hidden", "true"); syncModalBodyScroll(); }
  };

  const openLevelUpgradeModal = async () => {
    ensureLevelUpgradeModal();
    const m = document.getElementById("level-upgrade-modal");
    m.classList.add("is-open");
    m.setAttribute("aria-hidden", "false");
    syncModalBodyScroll();
    await renderLevelUpgradePlans();
  };

  const renderLevelUpgradePlans = async () => {
    const grid = document.getElementById("level-upgrade-plan-grid");
    if (!grid) return;
    const courseUpper = String(currentCourse || "").toUpperCase();

    // Ensure a hidden phantom #upgrade-modal exists so renderUpgradeSubscriptions
    // (the same function used on the home page) can populate it with correct prices.
    ...
    // Load + render into the phantom modal using the EXACT same logic as the home page
    await loadAndRenderSubscriptionOverrides();

    // Now copy only the cards that match the current level
    const sourceGrid = document.querySelector("#upgrade-modal .upgrade-plan-grid");
    if (sourceGrid) {
      const matchingCards = Array.from(sourceGrid.querySelectorAll(".upgrade-plan-card")).filter(card => {
        const h4 = card.querySelector(".upgrade-plan-head h4");
        return h4 && h4.textContent.trim().toUpperCase() === courseUpper;
      });
      if (matchingCards.length) {
        grid.innerHTML = `<div class="upgrade-plan-row">${matchingCards.map(c => c.outerHTML).join("")}</div>`;
        grid.querySelectorAll(".upgrade-plan-price-btn").forEach(btn => {
          if (btn.dataset.boundUpgradePrice !== "1") {
            btn.dataset.boundUpgradePrice = "1";
            btn.addEventListener("click", () => openEnrollModal(btn.dataset.level || "", btn.dataset.price || ""));
          }
        });
        startSubscriptionDiscountTimers();
        return;
    ...
    // Fallback: use static plan cards from index.html if it exists on this page
    if (!document.querySelector("#upgrade-modal .upgrade-plan-card")) {
      const mainGrid = document.querySelector("#upgrade-modal .upgrade-plan-grid");
      if (mainGrid) {
        const allCards = Array.from(mainGrid.querySelectorAll(".upgrade-plan-card"));
        const filtered = allCards.filter(card => {
          const nameEl = card.querySelector(".upgrade-plan-head h4");
          return nameEl && String(nameEl.textContent || "").trim().toUpperCase() === courseUpper;
        });
        if (filtered.length) {
          grid.innerHTML = `<div class="upgrade-plan-row">${filtered.map(c => c.outerHTML).join("")}</div>`;
          grid.querySelectorAll(".upgrade-plan-price-btn").forEach(btn => {
            btn.addEventListener("click", () => openEnrollModal(btn.dataset.level || "", btn.dataset.price || ""));
          });
          return;
    ...
    grid.innerHTML = `<p style="text-align:center;padding:24px;color:#4a5f76;">No plans available for ${courseUpper} at the moment.<br>Please contact us via Telegram.</p>`;
  };

  if (!isLoggedIn || hasLevelMismatch) {
    injectLevelUpgradeBanner();
    lessonButtons.forEach((button) => {
      resetLessonButton(button);
...
(() => {
  ...
  document.addEventListener("keydown", (e) => { if (e.key === "Escape" && modal.classList.contains("is-open")) closeComingSoonModal(); });
})();

const IN_DEV_COURSES = ["a2", "b1", "b2"];
...
const applyCourseLevelLocks = () => {
  ...
  courseButtons.forEach((button) => {
    ...
    // Courses still in development are always locked for everyone except admin.
    if (courseMatch && IN_DEV_COURSES.includes(courseMatch) && !isAdmin) {
      button.href = "#";
      button.textContent = "Locked";
      button.classList.add("is-locked-course-btn");
      button.setAttribute("aria-disabled", "true");
      button.setAttribute("tabindex", "-1");
      button.onclick = (e) => { e.preventDefault(); openComingSoonModal(); };
      return;
    }

    if (authState && authState.sessionToken && userLevel && courseMatch && courseMatch !== userLevel && !isAdmin) {
      button.href = "#";
      button.textContent = "Locked";
      button.classList.add("is-locked-course-btn");
      button.setAttribute("aria-disabled", "true");
...
const mobileNavClose = document.getElementById("mobile-nav-drawer-close");
const mobileNavGreeting = document.getElementById("mobile-nav-greeting");
...
if (drawerAdminBtn) {
  // link navigates naturally — just close drawer first
  drawerAdminBtn.addEventListener("click", () => closeMobileDrawer());
}
if (drawerLogoutBtn) {
...
const updateLoginButton = () => {
  ...
  if (heroHeader) {
    if (mobile && authState) {
      heroHeader.classList.add("mobile-logged-in");
    } else {
      heroHeader.classList.remove("mobile-logged-in");
    }
  }
  if (mobileMenuBtn) {
    mobileMenuBtn.hidden = !(mobile && !!authState);
  }

  // Update drawer greeting + admin button visibility
  if (mobileNavGreeting) {
    if (authState && authState.username) {
      const displayName = String(authState.fullName || "").trim() || authState.username;
      mobileNavGreeting.textContent = `Hi, ${displayName}`;
      mobileNavGreeting.hidden = false;
    } else {
      mobileNavGreeting.textContent = "";
      mobileNavGreeting.hidden = true;
    }
  }
  if (drawerAdminBtn) {
    drawerAdminBtn.hidden = !authState || !isAdminUser();
  }
  if (drawerProfileBtn) {
    // Show Profile in drawer only for non-admin users
    drawerProfileBtn.hidden = !authState || isAdminUser();
  }
...
if (resultsMobileMasonryEls.length) {
  ...
  const scheduleRelayout = (masonryEl) => {
    ...
    resultsMobileMasonryEls.forEach((masonryEl) => {
	  ...
	  window.__cxBuildResultsMasonry = buildMasonry;
...
const ensureTeamModals = () => {
  ...
  wrap.innerHTML = `
    <div id="team-modal" class="team-modal" aria-hidden="true">
      <div class="team-modal-card" role="dialog" aria-modal="true" aria-label="Team of Citizen X">
        <div class="team-modal-topbar">
          <div class="team-modal-topbar-spacer" aria-hidden="true"></div>
          <div class="team-modal-topbar-center">
            <img src="assets/images/logo.png" alt="Citizen X" class="team-modal-logo">
      ...
      <div class="admin-lessons-modal-card" role="dialog" aria-modal="true" aria-label="Edit team member">
        ...
        <div class="admin-team-edit-grid">
          ...
          <div class="admin-team-edit-fields">
            ...
            <input id="admin-team-edit-header" class="admin-lessons-input" type="text" placeholder="Citizen X - Founder & Lead Teacher">
...
const initTeamModal = () => {
  ...
  // Session-level flag: PIN verified once per page-load for team edit
  let teamEditVerified = false;
  ...
  const startGate = async () => {
    ...
    // If already verified this session (same page load), skip the gate
    if (teamEditVerified) {
      await openManagerModal();
      return;
    }

    // Always show the PIN gate — never bypass via localStorage for team edit
  ...
  gateSubmit.addEventListener("click", async () => {
    ...
    const verify = await verifyCode(code);
    if (verify.ok && verify.data && verify.data.ok) {
      teamEditVerified = true; // bypass gate for rest of this page session
...
(() => {
  ...
  const showReauthPrompt = (statusEl, type) => {
    ...
    btn.addEventListener("click", () => {
      ...
      openDailyPinModal();
      sendPin();
      window.__cxDailyReauthType = type;
    });
    ...
    pinSubmitBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (res.ok && res.data && res.data.ok) {
          ...
          const typeToOpen = window.__cxDailyReauthType || _editType;
          window.__cxDailyReauthType = null;
          closeDailyPinModal();
...
(function initPartnersSection() {
  ...
  const renderPartners = (items) => {
    ...
    items.forEach((p) => {
      ...
      if (p.logo_url) {
        const logoSrc = /^https?:\/\//i.test(p.logo_url) ? p.logo_url : `${API_BASE_URL}${p.logo_url}`;
        el.innerHTML = `<img class="partner-logo" src="${escapeHtml(logoSrc)}" alt="${escapeHtml(p.name)}" loading="lazy"><div class="partner-info"><span class="partner-name">${escapeHtml(p.name)}</span><span class="partner-type">Partner School</span></div>`;
      } else {
        el.innerHTML = `<div class="partner-logo-placeholder">🏫</div><div class="partner-info"><span class="partner-name">${escapeHtml(p.name)}</span><span class="partner-type">Partner School</span></div>`;
      }
      list.appendChild(el);
    });
  };

  // ── Partners cache (stale-while-revalidate) ──
  const PARTNERS_CACHE_KEY = "cx:partners:v1";
  const PARTNERS_CACHE_TTL = 10 * 60 * 1000; // 10 min

  const readPartnersCache = () => {
    try {
      const raw = localStorage.getItem(PARTNERS_CACHE_KEY);
      if (!raw) return null;
      const { data, ts } = JSON.parse(raw);
      return Array.isArray(data) ? { data, ts } : null;
    } catch (_) { return null; }
  };

  const writePartnersCache = (data) => {
    try { localStorage.setItem(PARTNERS_CACHE_KEY, JSON.stringify({ data, ts: Date.now() })); } catch (_) {}
  };

  const clearPartnersCache = () => {
    try { localStorage.removeItem(PARTNERS_CACHE_KEY); } catch (_) {}
  };

  // ── Loading indicator (animated dots) ──
  let _dotsTimer = null;

  const showPartnersLoading = () => {
    // Only show if nothing is rendered yet
    if (list.children.length > 0) return;
    strip.hidden = false;
    const el = document.createElement("div");
    el.className = "partners-loading";
    el.id = "partners-loading-indicator";
    ...
    _dotsTimer = setInterval(() => {
      dots = (dots % 3) + 1;
      if (el.isConnected) el.textContent = texts[dots - 1];
    }, 500);
  };

  const hidePartnersLoading = () => {
    clearInterval(_dotsTimer);
    _dotsTimer = null;
    const el = document.getElementById("partners-loading-indicator");
    if (el) el.remove();
  };

  // ── Load partners from API (with instant cache) ──
  const loadPartners = async ({ forceRefresh = false } = {}) => {
    // 1. Show cached data immediately — zero wait
    const cached = readPartnersCache();
    if (cached && cached.data.length > 0) {
      partnersData = cached.data;
      strip.hidden = false;
      renderPartners(partnersData);
      // If cache is still fresh and this isn't a forced admin reload, skip network
      if (!forceRefresh && (Date.now() - cached.ts) < PARTNERS_CACHE_TTL) return;
      // Stale cache — refresh silently in background (no loading indicator since data visible)
    } else {
      // No cache at all — show loading animation
      showPartnersLoading();
    }

    // 2. Background refresh from Railway
    try {
      await ensureApiBaseUrl();
      const res = await fetch(`${API_BASE_URL}/api/partners`);
      if (!res.ok) { hidePartnersLoading(); return; }
      ...
      writePartnersCache(fresh); // update cache
    } catch (_e) { hidePartnersLoading(); }
  };
  ...
  maybeShowEditBtn();
  window.addEventListener("cx:auth-changed", maybeShowEditBtn);
  ...
  // ── PIN gate ──
  const gateModal = document.getElementById("partners-gate-modal");
  const gateInput = document.getElementById("partners-gate-input");
  const gateStatus = document.getElementById("partners-gate-status");
  const gateResend = document.getElementById("partners-gate-resend");
  const gateSubmit = document.getElementById("partners-gate-submit");
  const gateClose = document.getElementById("partners-gate-close");

  const openGate = async () => {
    const authState = getAuthState();
    if (!authState || authState.role !== "admin") return;
    ...
    if (stored) {
      try {
        const vRes = await adminPostJson("/api/admin/edit-access/verify", { admin_username: authState.username, code: stored });
        if (vRes.ok && vRes.data && vRes.data.ok) { openModal(); return; }
      } catch (_) {}
      try { localStorage.removeItem(storageKey); } catch (_) {}
    }
    // Show gate modal and request PIN
    if (!gateModal) { openModal(); return; }
    gateModal.classList.add("is-open");
    gateModal.setAttribute("aria-hidden", "false");
    syncModalBodyScroll();
    if (gateInput) { gateInput.value = ""; gateInput.focus(); }
    if (gateStatus) gateStatus.textContent = "Sending PIN...";
    try {
      const rRes = await adminPostJson("/api/admin/edit-access/request", { admin_username: authState.username });
      if (gateStatus) gateStatus.textContent = rRes.ok ? "PIN sent. Check Telegram." : (rRes.data && rRes.data.error) || "request_failed";
    } catch (_) {
      if (gateStatus) gateStatus.textContent = "request_failed";
    }
  };
  ...
  const submitGate = async () => {
    const code = String(gateInput ? gateInput.value : "").trim();
    if (!code) return;
    const authState = getAuthState();
    if (!authState) return;
    if (gateStatus) gateStatus.textContent = "Verifying...";
    try {
      const vRes = await adminPostJson("/api/admin/edit-access/verify", { admin_username: authState.username, code });
      if (vRes.ok && vRes.data && vRes.data.ok) {
        try { localStorage.setItem(`${ADMIN_EDIT_CODE_STORAGE_PREFIX}${authState.username}`, code); } catch (_) {}
        closeGate();
        openModal();
      } else {
        const err = (vRes.data && vRes.data.error) || "invalid_code";
        if (gateStatus) gateStatus.textContent = err === "banned" ? "Too many attempts. Try later." : "Invalid code.";
      }
    } catch (_) {
      if (gateStatus) gateStatus.textContent = "Error.";
    }
  };

  if (gateClose) gateClose.addEventListener("click", closeGate);
  if (gateModal) gateModal.addEventListener("click", (e) => { if (e.target === gateModal) closeGate(); });
  if (gateResend) gateResend.addEventListener("click", async () => {
    const authState = getAuthState();
    if (!authState) return;
    if (gateStatus) gateStatus.textContent = "Sending PIN...";
    try {
  ...
  if (gateInput) gateInput.addEventListener("keydown", (e) => { if (e.key === "Enter") submitGate(); });

  editBtn.addEventListener("click", () => { openGate().catch(() => {}); });
  ...
  const renderAdminRows = () => {
    ...
    adminRows.forEach((row, idx) => {
      ...
      urlInput.className = "admin-lessons-input";
      urlInput.type = "url";
      urlInput.placeholder = "Website URL (https://...)";
      urlInput.value = row.url || "";
      urlInput.style.marginTop = "6px";
      urlInput.addEventListener("input", () => { adminRows[idx].url = urlInput.value; });
  ...
  if (addBtn) {
    addBtn.addEventListener("click", () => {
      ...
      adminRows.push({ id: newId, name: "", url: "", logo_file: "", logo_url: "", isNew: true });
      renderAdminRows();
      // Scroll to bottom
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const items = adminRows
          ...
          .map((r, i) => ({ id: r.id, name: r.name.trim(), url: String(r.url || "").trim(), logo_file: r.logo_file || "", order: i }));
        ...
        if (statusEl) statusEl.textContent = "Saved!";
        // Reload with cache bypass so admin sees fresh data immediately
        clearPartnersCache();
...
// Escape key for partners modals
document.addEventListener("keydown", (e) => {
  if (e.key !== "Escape") return;
  const gate = document.getElementById("partners-gate-modal");
  ...
  const adm = document.getElementById("partners-admin-modal");
  if (adm && adm.classList.contains("is-open")) {
...
(function initNews() {
  ...
  const feedLoad    = document.getElementById("news-feed-loading");
  const adminBar    = document.getElementById("news-admin-bar");
  const addBtn      = document.getElementById("news-add-btn");
  const editorWrap  = document.getElementById("news-editor-wrap");
  const editorLbl   = document.getElementById("news-editor-label");
  const editorClose = document.getElementById("news-editor-close");
  const editorArea  = document.getElementById("news-editor-area");
  const linkPopup   = document.getElementById("news-link-popup");
  const linkUrl     = document.getElementById("news-link-url-input");
  const linkAdd     = document.getElementById("news-link-add-btn");
  const linkCancel  = document.getElementById("news-link-cancel-btn");
  const imagePick   = document.getElementById("news-image-pick");
  const imageThumbs = document.getElementById("news-image-thumbs");
  const saveBtn     = document.getElementById("news-save-btn");
  const cancelBtn   = document.getElementById("news-cancel-btn");
  const pinWrap     = document.getElementById("news-pin-wrap");
  const pinClose    = document.getElementById("news-pin-close");
  const pinInput    = document.getElementById("news-pin-input");
  const pinStatus   = document.getElementById("news-pin-status");
  const pinSubmit   = document.getElementById("news-pin-submit");
  const pinResend   = document.getElementById("news-pin-resend");
  const badge       = document.getElementById("drawer-news-badge");
  const drawerBtn   = document.getElementById("drawer-news-btn");

  if (!overlay || !feed) return;

  /* ── State ───────────────────────────────────────────────────────────────── */
  let _news          = [];
  let _editId        = null;   // null = new post, string = editing existing
  let _adminVerified = false;
  let _pinCallback   = null;
  // Each item: { b64, ext, existing, url, fname }
  //   new images:      { b64:"data:...", ext:"jpg", existing:false }
  //   existing images: { url:"/api/news/image?file=x.jpg", fname:"x.jpg", existing:true }
  let _pendingImages = [];
  let _keepFiles     = [];     // fnames of existing images to keep on edit-save
  let _savedRange    = null;   // saved Selection range before opening link popup

  /* ── Seen / unread tracking ──────────────────────────────────────────────── */
  const SEEN_KEY = "cx_news_seen_v1";
  let _seen = new Set();
  try { _seen = new Set(JSON.parse(localStorage.getItem(SEEN_KEY) || "[]")); } catch (_) {}

  function saveSeen() {
    try { localStorage.setItem(SEEN_KEY, JSON.stringify([..._seen])); } catch (_) {}
  }
  function markSeen(id) {
    ...
    saveSeen();
    updateBadge();
    // remove unread highlight from the card
    const card = feed.querySelector(`[data-news-id="${id}"]`);
    if (card) card.classList.remove("news-card-unread");
  }
  function updateBadge() {
    const count = _news.filter(n => !_seen.has(n.id)).length;
    if (!badge) return;
  ...
  function isUserAdmin() {
    ...
    if (auth) return auth.role === "admin";
    // fallback
    const adminDrawerBtn = document.getElementById("drawer-admin-btn");
    if (adminDrawerBtn) return !adminDrawerBtn.hidden;
    return false;
  }

  /* ── Floating Telegram-style selection toolbar ───────────────────────────── */
  let _selToolbar = null;

  function getSelToolbar() {
    if (_selToolbar) return _selToolbar;
    _selToolbar = document.createElement("div");
    _selToolbar.className = "news-sel-toolbar";
    _selToolbar.setAttribute("role", "toolbar");
    _selToolbar.setAttribute("hidden", "");
    _selToolbar.innerHTML =
      '<button type="button" class="news-sel-btn" data-cmd="bold" title="Bold"><b>B</b></button>' +
      '<span class="news-sel-sep"></span>' +
      '<button type="button" class="news-sel-btn" data-cmd="italic" title="Italic"><i>I</i></button>' +
      '<span class="news-sel-sep"></span>' +
      '<button type="button" class="news-sel-btn" data-cmd="link" title="Add link">🔗</button>';
    document.body.appendChild(_selToolbar);

    // prevent mouse-down from collapsing selection
    _selToolbar.addEventListener("pointerdown", e => { e.preventDefault(); });

    _selToolbar.addEventListener("click", e => {
      const btn = e.target.closest("[data-cmd]");
      if (!btn) return;
      const cmd = btn.dataset.cmd;
      if (cmd === "bold")   { document.execCommand("bold");   hideSelToolbar(); return; }
      if (cmd === "italic") { document.execCommand("italic"); hideSelToolbar(); return; }
      if (cmd === "link") {
        const sel = window.getSelection();
        if (sel && sel.rangeCount > 0) _savedRange = sel.getRangeAt(0).cloneRange();
        hideSelToolbar();
        linkPopup.hidden = false;
        setTimeout(() => linkUrl && linkUrl.focus(), 60);
      }
    });
    return _selToolbar;
  }
  ...
  function positionSelToolbar() {
    ...
    if (!editorArea.contains(range.commonAncestorContainer)) { hideSelToolbar(); return; }
    const rect = range.getBoundingClientRect();
    if (!rect || rect.width === 0) { hideSelToolbar(); return; }
    const toolbar = getSelToolbar();
    toolbar.removeAttribute("hidden");
    // Position: centre above the selection
    // We need offsetHeight, so briefly make visible off-screen first if needed
  ...
  function hideSelToolbar() {
    if (_selToolbar) _selToolbar.setAttribute("hidden", "");
  }
  ...
  function renderPhotoGrid(imageUrls) {
    ...
    const showN    = Math.min(total, MAX_SHOW);

    const grid = document.createElement("div");
    grid.className = "news-photo-grid";
    grid.dataset.count = String(showN);

    imageUrls.slice(0, showN).forEach((url, i) => {
      const isLast    = i === showN - 1;
      const hasMore   = isLast && total > MAX_SHOW;

      const img = document.createElement("img");
      img.className = "news-grid-img";
      img.src       = url;
      img.alt       = "";
      img.loading   = "lazy";
      ...
      if (hasMore) {
        const wrap = document.createElement("div");
        wrap.className = "news-grid-more-wrap";
        const ov = document.createElement("div");
        ov.className   = "news-grid-more-overlay";
        ov.textContent = "+" + (total - MAX_SHOW + 1);
  ...
  function renderThumbs() {
    ...
    _pendingImages.forEach((item, idx) => {
      const wrap = document.createElement("div");
      wrap.className = "news-image-thumb";

      const img = document.createElement("img");
      img.src = item.url || item.b64 || "";
      img.alt = "";
      wrap.appendChild(img);

      const del = document.createElement("button");
      del.type      = "button";
      del.className = "news-image-thumb-del";
      del.textContent = "✕";
      del.title     = "Remove";
      del.addEventListener("click", () => {
        if (item.existing) {
          _keepFiles = _keepFiles.filter(f => f !== item.fname);
        }
  ...
  /* ── Feed renderer ───────────────────────────────────────────────────────── */
  function renderFeed() {
    // Clear both news cards AND any existing empty-state (prevents duplicates)
    feed.querySelectorAll(".news-card, .news-empty").forEach(el => el.remove());
    ...
    const showAdmin = isUserAdmin();

    // IntersectionObserver marks items as seen when 60% visible
    ...
    // News is already sorted newest-first from the backend
    _news.forEach(item => {
      const unread = !_seen.has(item.id);
      const card   = document.createElement("div");
      card.className   = "news-card" + (unread ? " news-card-unread" : "");
      card.dataset.newsId = item.id;

      // "🆕 NEW" badge
      if (unread) {
        const nb = document.createElement("div");
        nb.className   = "news-card-new-badge";
        nb.textContent = "🆕 NEW";
        card.appendChild(nb);
      }
      ...
      if (item.image_urls && item.image_urls.length > 0) {
        const grid = renderPhotoGrid(item.image_urls);
      ...
      } catch (_) { time.textContent = item.created_at || ""; }
      meta.appendChild(time);

      if (showAdmin) {
        const actions = document.createElement("div");
        actions.className = "news-card-admin-actions";

        const editBtn = document.createElement("button");
        editBtn.type      = "button";
        editBtn.className = "btn btn-xs";
        editBtn.textContent = "✏️ Edit";
        editBtn.addEventListener("click", () => requirePin(() => openEditor(item)));
        actions.appendChild(editBtn);

        const delBtn = document.createElement("button");
        delBtn.type      = "button";
        delBtn.className = "btn btn-xs btn-xs-red";
        delBtn.textContent = "🗑 Delete";
        delBtn.addEventListener("click", () => requirePin(() => deletePost(item.id)));
  ...
  async function loadNews() {
    if (feedLoad) feedLoad.hidden = false;
    try {
      if (typeof adminGetJson === "function") {
        const res = await adminGetJson("/api/news");
        _news = res.ok ? (res.data.items || []) : [];
      } else {
    ...
    if (feedLoad) feedLoad.hidden = true;
    renderFeed();
    updateBadge();
  }
  ...
  function openEditor(item) {
    ...
    editorLbl.textContent  = item ? "Edit post" : "New post";
    editorArea.innerHTML   = item ? (item.content || "") : "";
    ...
    // Populate existing images as thumbs (edit mode)
    if (item && item.image_urls && item.image_urls.length > 0) {
      item.image_urls.forEach(url => {
        // extract filename from ?file=FNAME
        const m = url.match(/[?&]file=([^&]*)/);
        const fname = m ? decodeURIComponent(m[1]) : "";
        if (fname) _keepFiles.push(fname);
        _pendingImages.push({ existing: true, url, fname });
      });
    }

    renderThumbs();
    editorWrap.hidden = false;
    setTimeout(() => editorArea.focus(), 60);
  }
  ...
  async function savePost() {
    ...
    if (!html && _pendingImages.length === 0) { editorArea.focus(); return; }
    saveBtn.disabled = true;

    const newImagesB64 = _pendingImages
      .filter(p => !p.existing)
      .map(p => ({ b64: p.b64.split(",")[1], ext: p.ext }));
    ...
    try {
      let endpoint, body;
      if (_editId) {
        endpoint = "/api/admin/news/update";
        body = {
          id:               _editId,
          content:          html,        // server expects "content"
          keep_image_files: _keepFiles,
          image_files_b64:  newImagesB64,
          admin_username:   auth ? auth.username : undefined
        };
      } else {
        endpoint = "/api/admin/news/create";
        body = {
          content:         html,         // server expects "content"
          image_files_b64: newImagesB64,
          admin_username:  auth ? auth.username : undefined
      ...
      const res = (typeof adminPostJson === "function")
        ? await adminPostJson(endpoint, body, 30000)  // 30s timeout for image uploads
        : await fetch(endpoint, { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async r => ({ ok: r.ok, status: r.status, data: await r.json() }));

      if (!res.ok) throw new Error((res.data && res.data.error) || "HTTP " + res.status);
      const saved = res.data.item;  // server returns { ok, item: {...} }

      if (_editId) {
        const idx = _news.findIndex(n => n.id === _editId);
        if (idx !== -1) _news[idx] = saved;
      } else {
        _news.unshift(saved);
      }
  ...
  async function deletePost(id) {
    ...
    try {
      const res = (typeof adminPostJson === "function")
        ? await adminPostJson("/api/admin/news/delete", { id, admin_username: auth ? auth.username : undefined })
        : await fetch("/api/admin/news/delete", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) }).then(r => ({ ok: r.ok }));
  ...
  async function requestPin() {
    pinStatus.textContent = "Sending PIN…";
    const auth = (typeof getAuthState === "function") ? getAuthState() : null;
    const username = auth ? auth.username : "";
    try {
      // Must use adminPostJson so X-Session-Token header is included
      const res = (typeof adminPostJson === "function")
        ? await adminPostJson("/api/admin/edit-access/request", { admin_username: username })
        : { ok: false, data: {} };

      if (!res.ok) {
        const errCode = res.data && res.data.error;
        if (errCode === "telegram_bot_not_configured") {
          pinStatus.textContent = "Telegram bot not configured on server.";
        } else if (errCode === "unauthorized" || res.status === 401) {
          pinStatus.textContent = "Session expired — please log in again.";
        } else if (errCode === "banned") {
          pinStatus.textContent = "Too many attempts. Try later.";
        } else {
          pinStatus.textContent = "Failed to send PIN. Tap Resend.";
        }
        return;
      }
      pinStatus.textContent = "✓ PIN sent to Telegram.";
      pinStatus.dataset.ok = "true";
      setTimeout(() => { if (pinStatus) { pinStatus.textContent = ""; delete pinStatus.dataset.ok; } }, 3000);
    } catch (_) {
      pinStatus.textContent = "Network error. Tap Resend.";
    }
  }

  async function verifyPin() {
    const pin = (pinInput.value || "").trim();
    if (pin.length < 4) { pinStatus.textContent = "Enter the PIN."; return; }
    pinStatus.textContent = "Verifying…";
    const auth = (typeof getAuthState === "function") ? getAuthState() : null;
    const username = auth ? auth.username : "";
    try {
      const res = (typeof adminPostJson === "function")
        ? await adminPostJson("/api/admin/edit-access/verify", { code: pin, admin_username: username })
        : { ok: false, data: {} };

      if (!res.ok || !res.data.ok) {
        pinStatus.textContent = (res.data && res.data.message) || "Incorrect PIN.";
        return;
      }
      _adminVerified = true;
      pinWrap.hidden = true;
  ...
  function openModal() {
    ...
    overlay.style.display = "";          // clear inline "none" from closeModal
    pinWrap.style.display = "";          // restore pin wrap to its CSS default
    overlay.setAttribute("aria-hidden", "false");
    document.body.style.overflow = "hidden";
    adminBar.hidden = !isUserAdmin();
    loadNews();
  }

  function closeModal() {
    overlay.hidden        = true;
    overlay.style.display = "none";      // belt-and-suspenders
    pinWrap.hidden        = true;
    pinWrap.style.display = "none";      // belt-and-suspenders for fixed PIN overlay
    overlay.setAttribute("aria-hidden", "true");
    document.body.style.overflow = "";
    closeEditor();
  ...
  /* ── Event wiring ────────────────────────────────────────────────────────── */
  // Open via drawer button
  if (drawerBtn) {
    drawerBtn.addEventListener("click", () => {
      // Use the proper close function so backdrop is also hidden
      if (typeof closeMobileDrawer === "function") closeMobileDrawer();
      openModal();
    });
  }

  // Close overlay — stopPropagation so click never reaches the overlay listener
  closeBtn && closeBtn.addEventListener("click", e => { e.stopPropagation(); closeModal(); });
  // Click on the dark backdrop (not the dialog) also closes
  overlay.addEventListener("click", e => {
    if (!e.target.closest(".news-dialog")) closeModal();
  });

  // Editor controls
  addBtn      && addBtn.addEventListener("click",  () => requirePin(() => openEditor(null)));
  editorClose && editorClose.addEventListener("click", closeEditor);
  cancelBtn   && cancelBtn.addEventListener("click", closeEditor);
  saveBtn     && saveBtn.addEventListener("click",  savePost);

  // Multi-image picker
  if (imagePick) {
    imagePick.addEventListener("change", async () => {
      const files = Array.from(imagePick.files || []);
      for (const file of files) {
        if (_pendingImages.length >= 10) break; // hard cap: 10 images per post
        ...
        _pendingImages.push({ b64, ext, existing: false });
      }
      imagePick.value = "";
      renderThumbs();
    });
  ...
  if (linkAdd) {
    linkAdd.addEventListener("click", () => {
      ...
      editorArea.querySelectorAll("a").forEach(a => {
        a.setAttribute("target", "_blank");
        a.setAttribute("rel",    "noopener noreferrer");
      });
      linkPopup.hidden = true;
  ...
  if (linkUrl) {
    linkUrl.addEventListener("keydown", e => {
      if (e.key === "Enter") { e.preventDefault(); linkAdd && linkAdd.click(); }
    });
  }

  // PIN dialog
  pinClose  && pinClose.addEventListener("click",  () => { pinWrap.hidden = true; _pinCallback = null; });
  pinSubmit && pinSubmit.addEventListener("click",  verifyPin);
  pinResend && pinResend.addEventListener("click",  requestPin);
  pinInput  && pinInput.addEventListener("keydown", e => { if (e.key === "Enter") verifyPin(); });

  // Escape key handling (news-scoped, won't conflict with other modals)
  document.addEventListener("keydown", e => {
    if (e.key !== "Escape") return;
    if (overlay.hasAttribute("hidden")) return; // news modal not open — ignore
    if (!pinWrap.hidden)    { pinWrap.hidden = true; _pinCallback = null; return; }
    if (linkPopup && !linkPopup.hidden) { linkCancel && linkCancel.click(); return; }
    if (!editorWrap.hidden) { closeEditor(); return; }
    closeModal();
  });

  // Initial badge update (0 until first loadNews call)
  updateBadge();
})();
// ── END News ──────────────────────────────────────────────────────────────────

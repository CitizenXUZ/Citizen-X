...
This guide provides step-by-step instructions for deploying the Citizen X LMS architecture.
...
4. **Set Environment Variables**:
   ...
   * Add the following keys:
     * `CX_DB_DIR` = `/data` *(This tells Python to save the DB and uploads into the persistent Volume)*
     * `CX_ADMIN_USERNAME` = `your_secure_admin_username`
     * `CX_ADMIN_PASSWORD` = `your_secure_admin_password`
     * `CX_ADMIN_EDIT_CODE` = `your_secure_edit_code` *(optional)*
5. **Get Backend URL**:
   * Once deployed, Railway will generate a public URL under the **Settings** -> **Domains** tab (e.g., `https://cx-api.up.railway.app`). Copy this URL.
...
1. **Configure API Proxy (`vercel.json`)**:
   ...
   * Example: `"destination": "https://cx-api.up.railway.app/api/$1"`

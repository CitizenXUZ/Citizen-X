...
DEFAULT_DB_DIR = Path(os.environ.get("CX_DB_DIR") or (Path.home() / ".cx"))
DEFAULT_DB_DIR.mkdir(parents=True, exist_ok=True)
DB_PATH = DEFAULT_DB_DIR / "database.db"
OPEN_ACCESS = os.environ.get("CX_OPEN_ACCESS", "").strip().lower() in {"1", "true", "yes", "on"}
if LEGACY_DB_PATH.exists() and not DB_PATH.exists():
    try:
        shutil.copy2(LEGACY_DB_PATH, DB_PATH)
    except OSError:
        pass
HOST = os.environ.get("CX_HOST", "0.0.0.0")
try:
    PORT = int(os.environ.get("CX_PORT") or os.environ.get("PORT") or "8000")
...
LOCAL_TZ_OFFSET_HOURS = int(os.environ.get("CX_TZ_OFFSET_HOURS", "5"))
...
TRANSLATE_URL = os.environ.get("CX_TRANSLATE_URL", "").strip()
TRANSLATE_API_KEY = os.environ.get("CX_TRANSLATE_API_KEY", "").strip()
...
ADMIN_USERNAME = os.environ.get("CX_ADMIN_USERNAME", "azamat_admin").strip()
RAW_ADMIN_PASSWORD = os.environ.get("CX_ADMIN_PASSWORD", "").strip()
ADMIN_PASSWORD = RAW_ADMIN_PASSWORD
if not ADMIN_PASSWORD:
    ADMIN_PASSWORD = secrets.token_urlsafe(24)
    print(f"WARNING: CX_ADMIN_PASSWORD not set. Generated temporary password: {ADMIN_PASSWORD}")
    print("Set CX_ADMIN_PASSWORD in your .env file for a persistent password.")
ADMIN_EDIT_PERMANENT_CODE = (os.environ.get("CX_ADMIN_EDIT_CODE") or "").strip() or ADMIN_PASSWORD
...
NEWS_PATH = DEFAULT_DB_DIR / "news.json"
NEWS_IMAGE_UPLOAD_DIR = DEFAULT_DB_DIR / "uploads" / "news"
NEWS_IMAGE_UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
...
DEFAULT_TEAM_MEMBERS: list[dict] = [
    {
        ...
        "telegram_username": "Citizen_Xbot",
    ...
    },
    {
        ...
        "subheader": "Leads development, ensures code quality, and turns ideas into working products. Creator of the platform Citizen X and other projects.",
        "achievements": [
...
def sanitize_news_html(content: str) -> str:
    """Strip dangerous HTML while keeping safe formatting + anchor tags."""
    ...
    content = re.sub(r"(href|src)\s*=\s*[\"']?\s*javascript:[^\"'>\s]*[\"']?", "", content, flags=re.IGNORECASE)
...
def load_news() -> list[dict]:
    if not NEWS_PATH.exists():
        return []
    try:
        raw_text = NEWS_PATH.read_text(encoding="utf-8-sig")
        payload = json.loads(raw_text)
    except (OSError, json.JSONDecodeError):
        return []
...
def save_news(items: list[dict]) -> bool:
    try:
        atomic_write_text(NEWS_PATH, json.dumps(items, ensure_ascii=False, indent=2))
        return True
    except OSError:
        return False
...
def init_db():
    ...
    cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='user_parents'")
    if cur.fetchone():
    ...
    # Seed admin account — azamat_admin
    _ADMIN_USERNAME = "azamat_admin"
    _ADMIN_PASSWORD = "AA20080608zz"
    _ADMIN_PHONE    = "+998953103443"
    _ADMIN_NAME     = "Azamat Allaberdiev"
    cur.execute("SELECT id, password_hash, salt FROM users WHERE username = ?", (_ADMIN_USERNAME,))
    _admin_row = cur.fetchone()
    if _admin_row is None:
        _salt = secrets.token_hex(16)
        _hash = hash_password(_ADMIN_PASSWORD, _salt)
        cur.execute(
            """
            INSERT INTO users (full_name, username, password, password_hash, salt, role, phone, created_at, access_started_at)
            VALUES (?, ?, '', ?, ?, 'admin', ?, ?, ?)
            """,
            (_ADMIN_NAME, _ADMIN_USERNAME, _hash, _salt, _ADMIN_PHONE,
             datetime.now(timezone.utc).isoformat(), datetime.now(timezone.utc).isoformat()),
        )
    else:
        # Keep credentials in sync with the values hardcoded above
        _stored_hash = str(_admin_row["password_hash"] or "").strip()
        _stored_salt = str(_admin_row["salt"] or "").strip()
        if not _stored_salt or hash_password(_ADMIN_PASSWORD, _stored_salt) != _stored_hash:
            _new_salt = secrets.token_hex(16)
            _new_hash = hash_password(_ADMIN_PASSWORD, _new_salt)
            cur.execute(
                "UPDATE users SET full_name = ?, password = '', password_hash = ?, salt = ?, role = 'admin', phone = ? WHERE id = ?",
                (_ADMIN_NAME, _new_hash, _new_salt, _ADMIN_PHONE, _admin_row["id"]),
            )
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/partners":
            ...
            for idx, item in enumerate(partners):
                ...
                partner_url = str(item.get("url", "") or "").strip()
                safe.append({
                    "id": pid,
                    "name": name,
                    "url": partner_url,
        ...
        if parsed.path == "/api/news":
            ...
            for item in news_items:
                ...
                if not nid:
                    continue
                # Support both legacy image_file (str) and new image_files (list)
                raw_files = item.get("image_files", None)
                if raw_files is None:
                    old_f = str(item.get("image_file", "") or "").strip()
                    raw_files = [old_f] if old_f else []
                image_urls = [
                    f"/api/news/image?file={quote(str(f))}"
                    for f in raw_files if isinstance(f, str) and f.strip()
                ]
                result.append({
                    "id": nid,
            ...
            self._set_headers(200)
            self.wfile.write(json.dumps({"items": result}).encode("utf-8"))
            return

        if parsed.path == "/api/news/image":
            params = {key: values[0] for key, values in parse_qs(parsed.query).items()}
            # New: ?file=<filename>  (legacy: ?id=<newsId> still supported)
            fname = str(params.get("file", "") or "").strip()
            if not fname:
                # legacy id-based lookup
                nid = str(params.get("id", "") or "").strip()
                if not nid:
                    self._set_headers(400)
                    self.wfile.write(json.dumps({"error": "file or id is required"}).encode("utf-8"))
                    return
                news_items = load_news()
                match_item = next((n for n in news_items if isinstance(n, dict) and str(n.get("id", "") or "").strip() == nid), None)
                if match_item is None:
                    self._set_headers(404)
                    return
                fname = str(match_item.get("image_file", "") or "").strip()
                if not fname:
                    self._set_headers(404)
                    return
            file_path = (NEWS_IMAGE_UPLOAD_DIR / fname).resolve()
            try:
                file_path.relative_to(NEWS_IMAGE_UPLOAD_DIR.resolve())
            except ValueError:
                self._set_headers(403)
                return
            if not file_path.exists():
    ...
    def do_POST(self):
        ...
        if self.path == "/api/admin/partners/set-all":
            ...
            for idx, item in enumerate(raw_items[:200]):
                ...
                partner_url = str(item.get("url", "") or "").strip()
                if len(partner_url) > 500:
                    partner_url = partner_url[:500]
                next_items.append({"id": pid, "name": name, "url": partner_url, "logo_file": logo_file, "order": idx})
        ...
        if self.path == "/api/admin/news/create":
            data = self._read_json()
            ...
            if not self._require_admin(conn):
                ...
                return
            conn.close()
            # Accept image_files_b64: [{b64, ext}, ...] — up to 10 images
            raw_imgs = data.get("image_files_b64", [])
            if not isinstance(raw_imgs, list):
                raw_imgs = []
            saved_files = []
            for img_item in raw_imgs[:10]:
                if not isinstance(img_item, dict):
                    continue
                b64 = str(img_item.get("b64", "") or "").strip()
                ext = str(img_item.get("ext", ".jpg") or ".jpg").strip().lower()
                ...
                if b64:
                    try:
                        ...
                        (NEWS_IMAGE_UPLOAD_DIR / fname).write_bytes(img_bytes)
                        saved_files.append(fname)
                    except Exception:
                        pass
            content = sanitize_news_html(content)
            now_str = utc_now().astimezone(LOCAL_TZ).isoformat()
            ...
            if not save_news(news_list):
                self._set_headers(500)
                self.wfile.write(json.dumps({"error": "save failed"}).encode("utf-8"))
                return
            image_urls = [f"/api/news/image?file={quote(f)}" for f in saved_files]
            self._set_headers(200)
            self.wfile.write(json.dumps({
                "ok": True,
                "item": {"id": nid, "content": content, "image_urls": image_urls,
                         "created_at": now_str, "updated_at": now_str},
            }).encode("utf-8"))
            return

        if self.path == "/api/admin/news/update":
            data = self._read_json()
            nid = str(data.get("id", "") or "").strip()
            content = str(data.get("content", "") or "").strip()
            if not nid or not content:
                self._set_headers(400)
                self.wfile.write(json.dumps({"error": "id and content are required"}).encode("utf-8"))
                return
            conn = get_connection()
            if not self._require_admin(conn):
                conn.close()
            ...
            conn.close()
            news_list = load_news()
            match_item = next((n for n in news_list if isinstance(n, dict) and str(n.get("id", "") or "").strip() == nid), None)
            if match_item is None:
                self._set_headers(404)
                self.wfile.write(json.dumps({"error": "news not found"}).encode("utf-8"))
                return
            # Normalise existing image files (backward compat)
            existing_files = match_item.get("image_files", None)
            if existing_files is None:
                old_single = str(match_item.get("image_file", "") or "").strip()
                existing_files = [old_single] if old_single else []
            # Images to keep (filenames sent back from client, already uploaded)
            keep_files = data.get("keep_image_files", None)
            if keep_files is None:
                keep_files = existing_files  # keep all if not specified
            # Delete files that are no longer kept
            for f in existing_files:
                if f not in keep_files:
                    try:
                        (NEWS_IMAGE_UPLOAD_DIR / f).unlink(missing_ok=True)
                    except Exception:
                        pass
            # Upload new images
            raw_imgs = data.get("image_files_b64", [])
            ...
            for img_item in raw_imgs[:10]:
                ...
                if b64:
                    try:
                        ...
                        pass
            final_files = [f for f in keep_files if f] + new_files
            content = sanitize_news_html(content)
            now_str = utc_now().astimezone(LOCAL_TZ).isoformat()
            match_item["content"] = content
            match_item["image_files"] = final_files
            match_item.pop("image_file", None)  # remove legacy field
            match_item["updated_at"] = now_str
            if not save_news(news_list):
                self._set_headers(500)
                self.wfile.write(json.dumps({"error": "save failed"}).encode("utf-8"))
                return
            image_urls = [f"/api/news/image?file={quote(f)}" for f in final_files]
            self._set_headers(200)
            self.wfile.write(json.dumps({
                "ok": True,
                "item": {"id": nid, "content": content, "image_urls": image_urls,
                         "created_at": str(match_item.get("created_at", "")), "updated_at": now_str},
            }).encode("utf-8"))
            return

        if self.path == "/api/admin/news/delete":
            data = self._read_json()
            nid = str(data.get("id", "") or "").strip()
            if not nid:
                self._set_headers(400)
            ...
            all_files = match_item.get("image_files", None)
            if all_files is None:
                old_f = str(match_item.get("image_file", "") or "").strip()
                all_files = [old_f] if old_f else []
            for f in all_files:
                try:
                    (NEWS_IMAGE_UPLOAD_DIR / f).unlink(missing_ok=True)
                except Exception:
                    pass
            news_list = [n for n in news_list if not (isinstance(n, dict) and str(n.get("id", "") or "").strip() == nid)]
            if not save_news(news_list):
                self._set_headers(500)
                self.wfile.write(json.dumps({"error": "save failed"}).encode("utf-8"))
                return
...
def main():
    ...
    parser = argparse.ArgumentParser(description="CX API server")
